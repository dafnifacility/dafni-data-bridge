...
def calc_demands(YEAR, MONTH, coeffs_country, GB_daily_HDD_ERA5, GB_daily_CDD_ERA5):
    ...
    weather_dep_demand = (
        coeffs_country[0] * 2021 + coeffs_country[8] * GB_daily_HDD_ERA5 + coeffs_country[9] * GB_daily_CDD_ERA5
    )
    full_demand = (
        coeffs_country[0] * 2021
        + coeffs_country[8] * GB_daily_HDD_ERA5
        + coeffs_country[9] * GB_daily_CDD_ERA5
        + coeffs_country[1] * pred_WD1
        + coeffs_country[2] * pred_WD2
        + coeffs_country[3] * pred_WD3
        + coeffs_country[4] * pred_WD4
        + coeffs_country[5] * pred_WD5
        + coeffs_country[6] * pred_WK1
        + coeffs_country[7] * pred_WK2
    )
    ...
    for YEAR in range(2025, 2026):
        ...
        for MONTH in ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"]:
            ...
            weights_da = xr.DataArray(
                weights,
                coords=[daily_mean_field["day_of_year"], daily_mean_field["latitude"], daily_mean_field["longitude"]],
                dims=["day_of_year", "latitude", "longitude"],
            )

            # Calculate weighted spatial mean for each day
            daily_timeseries_xarray = (daily_mean_field * weights_da).sum(
                dim=["latitude", "longitude"]
            ) / weights_da.sum(dim=["latitude", "longitude"])
            ...
            GB_daily_weather_dependent_demand_ERA5, GB_daily_full_demand_ERA5 = calc_demands(
                YEAR, MONTH, coeffs_country, GB_daily_HDD_ERA5, GB_daily_CDD_ERA5
            )
            ...
            iris.save(
                GB_daily_weather_dependent_demand_ERA5,
                "/gws/pw/j07/ceraf/hbloomfield01/Data/ERA5/ERA5_demand_and_national_aggs/ERA5_"
                + str(COUNTRY_nospace)
                + "_"
                + str(YEAR)
                + "_"
                + MONTH
                + "_wd_demand_timeseries.nc",
            )
            iris.save(
                GB_daily_full_demand_ERA5,
                "/gws/pw/j07/ceraf/hbloomfield01/Data/ERA5/ERA5_demand_and_national_aggs/ERA5_"
                + str(COUNTRY_nospace)
                + "_"
                + str(YEAR)
