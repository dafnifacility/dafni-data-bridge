...
├── cli/
│   ├── config_parser.py            # CLI argument parsing and config merging
│   └── main.py                     # Entry point
├── transport/
│   ├── auth.py                     # JWT token authentication
│   ├── session.py                  # HTTP session with retry logic
│   └── client.py                   # High-level client orchestrator
└── downloader/
    ├── __init__.py                 # get_downloader() factory
    ├── base.py                     # BaseDownloader ABC
    ├── models.py                   # DownloadResult dataclass
    ├── download_utils.py           # URL parsing and multi-download utilities
    ├── progress_logger.py          # Progress bar and logging callbacks
    ├── s3_upload.py                # S3 multipart upload client
    └── services/
        ├── http_service.py         # HTTP/HTTPS downloader
        ├── http_gws.py             # GWS-specific HTTP downloader
        ├── ftp_service.py          # FTP downloader
        └── ssh_service.py          # SSH/SFTP downloader
```
...
```
main()
  → ConfigLoader.parse()           # Parse CLI args + optional JSON config
  → Client.__init__() or classmethod # Set up auth + session
    → Auth + SessionManager         # Token validation + HTTP retry session
    → get_downloader(url, session)  # Factory: pick downloader by URL
  → Client.download()
    → ProgressLogger                # Set up progress bar
    → BaseDownloader.download()     # Template method
      → _stream() → _write_file()  # Single file to local disk
      → _stream() → s3_upload()    # Single file to S3
      → _recursive_download()      # Directory traversal
    → DownloadResult                # url, destination, size, checksum
```
